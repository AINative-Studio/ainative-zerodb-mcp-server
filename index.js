...
class ZeroDBMCPServer {
  constructor() {
    this.apiUrl = process.env.ZERODB_API_URL || 'https://api.ainative.studio/api/v1';
  ...
  async renewToken() {
    // Skip token renewal if API key is provided directly
    if (process.env.ZERODB_API_KEY) {
      console.error('Using provided API key, skipping authentication');
      this.apiToken = process.env.ZERODB_API_KEY;
      this.tokenExpiry = Date.now() + (365 * 24 * 60 * 60 * 1000); // Set far future expiry for API key
      return true;
    }
    ...
    try {
      ...
      const response = await axios.post(
        `${this.apiUrl.replace('/api/v1', '')}/api/v1/admin/auth/login`,
        new URLSearchParams({
  ...
  async getContext(args) {
    try {
      ...
      if (!Array.isArray(memories)) {
        throw new Error('Invalid response format: expected array of memories');
      }
  ...
  async start() {
    try {
      ...
      console.error('ZeroDB MCP Server v2.0.0 running on stdio');
